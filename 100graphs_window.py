...
от числа проведённых игр, сгруппированных с заданным окном (50 по умолчанию) для всех игроков из dataframe.
Результат записывается в dota_charts_window.html"""
...
for x in range(len(dframe[1].unique())):
    ...
    if len(player_dframe) > window:
        groupper = [item for sublist in [[x for y in range(window)] for x in range(len(player_dframe) // window)]
                for item in sublist]
...
list_unique_heroes_indexes = list([list_players[x][33].unique()*window+window for x in range(0, len(list_players))])

# Создаём Excel файл и записываем в него данные об игроках по окнам:

# Создаём книгу с названием вида "Дата Время Размер_окна.xlsx" и добавляем один лист
workbook = xlsxwriter.Workbook(datetime.now().strftime("%Y-%m-%d %H-%M ") + str(window) + '.xlsx')
worksheet = workbook.add_worksheet()

# Запись начинается из нулевой ячейки
row = 0

for x in range(0, len(list_players)):
    for y in range(0, len(list_unique_heroes[x])):
        # id игрока, номер игры и количествово уникальных героев
        print(dframe[1].unique()[x], "\t", int(list_players[x][33].unique()[y]), "\t", list_unique_heroes[x][y])

        worksheet.write(row, 0, dframe[1].unique()[x])
        worksheet.write(row, 1, int(list_players[x][33].unique()[y]))
        worksheet.write(row, 2, list_unique_heroes[x][y])
...
output_file("dota_charts_window.html", title="Выбор героев по заданному окну")
